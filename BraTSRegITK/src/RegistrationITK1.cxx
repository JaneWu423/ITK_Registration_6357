...
typedef itk::Image <float, nDims> TMovingImage;
typedef itk::Image <float, nDims> TFixedImage;
...
int main (int argc, char * argv[]) {
    if (argc < 7) {
        ...
        std::cerr << argv[0] << " inputMovingImageFile inputFixedImageFile segImageFile outputRegisteredMovingImageFile inputCSV outputCSV" << std::endl; 
        return EXIT_FAILURE;
    }
    ...
    /* rescale moving image intensity */
    using StatisticsFilterType = itk::StatisticsImageFilter<ImageType>;
    StatisticsFilterType::Pointer statisticsFilter = StatisticsFilterType::New();
    statisticsFilter->SetInput(fixedReader->GetOutput());
    statisticsFilter->Update();
    float minimumIntensity = statisticsFilter->GetMinimum();
    float maximumIntensity = statisticsFilter->GetMaximum();

    using RescaleFilterType = itk::RescaleIntensityImageFilter<ImageType, ImageType>;
    RescaleFilterType::Pointer rescaleFilter = RescaleFilterType::New();
    rescaleFilter->SetInput(movingReader->GetOutput());
    rescaleFilter->SetOutputMinimum(minimumIntensity);
    rescaleFilter->SetOutputMaximum(maximumIntensity);
    rescaleFilter->Update();

    /* tumor binary classification */
    using ThresholdFilterType = itk::BinaryThresholdImageFilter<ImageType, MaskImageType>;
    ThresholdFilterType::Pointer thresholdFilter = ThresholdFilterType::New();
    thresholdFilter->SetInput(rescaleFilter->GetOutput());
    thresholdFilter->SetLowerThreshold(1500.0);
    thresholdFilter->SetUpperThreshold(1900.0);
    ...
    using FixedImageMaskType = itk::ImageMaskSpatialObject<3>;
    using MovingImageMaskType = itk::ImageMaskSpatialObject<3>;

    FixedImageMaskType::Pointer fixedImageMask = FixedImageMaskType::New();
    fixedImageMask->SetImage(maskImage);
    MovingImageMaskType::Pointer movingImageMask = MovingImageMaskType::New();
